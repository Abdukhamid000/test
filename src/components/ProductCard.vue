    ...
    <div class="mt-4 px-5 pb-5">
      ...
      <div class="mt-2 mb-5 flex items-center justify-between">
        ...
        <div class="flex items-center">
          <div v-for="(icon, index) in starIcons">
            <svg
              :key="index"
              aria-hidden="true"
              class="h-5 w-5"
              :class="{ 'text-yellow-300': icon === 'filled' }"
              fill="currentColor"
              viewBox="0 0 20 20"
...
const starIcons = computed(() => {
  const filledStars = Math.floor(props.product.rating)
  const emptyStars = 5 - filledStars
  return (
    Array.from({ length: filledStars }, () => 'filled') +
